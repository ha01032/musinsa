...
def insertData(title, link, brand, price, star, starnum, imglink,num, pagenum) :
    ...
    try :
        ...
        print(data6)
        print(data7)
        sql = "INSERT INTO top (title,link,price,brand,star,starnum,imglink,updatedate) VALUES ('"+ data1 + "','" + data2 + "','" + data3 + "','" + data4 + "','"+ data5 +"','"+data6+ "','"+data7+ "', NOW())"
        print(sql)
        print("----------------------------------------")
        cur.execute(sql)
...
def resetData() :
    ...
    try :    
        ...
        cur.execute("ALTER TABLE top AUTO_INCREMENT=1") 
    except Exception as err:
        print("예외 발생")
        print(str(err))
    else :
...
## 전역 변수부
musinsaUrl = "https://www.musinsa.com/categories/item/001?d_cat_cd=001&brand=&list_kind=small&sort=sale_high&sub_sort=1m&display_cnt=90&group_sale=&exclusive_yn=&sale_goods=&timesale_yn=&ex_soldout=&kids=&color=&price1=&price2=&shoeSizeOption=&tags=&campaign_id=&includeKeywords=%EB%82%A8%EC%84%B1&measure=&page="
pagenum = 1
